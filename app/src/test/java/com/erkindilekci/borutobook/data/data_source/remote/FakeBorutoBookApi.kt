...
import com.erkindilekci.borutobook.data.data_source.remote.dto.toHeroDto
import com.erkindilekci.borutobook.domain.model.Hero

class FakeBorutoBookApi : BorutoBookApi {
    ...
    private val heroes = listOf(
        Hero(
            ...
            abilities = listOf("Sharingan", "Rinnegan", "Sussano", "Amateratsu", "Intelligence"),
        ...
        Hero(
            ...
            abilities = listOf("Chakra Control", "Medical Ninjutsu", "Strength", "Intelligence"),
    ...
    override suspend fun searchHeroes(name: String): ApiResponse {
        val searchedHeroes = findHeroes(name).map { it.toHeroDto() }
        return ApiResponse(
            success = true,
            message = "OK",
    ...
    private fun findHeroes(name: String): List<Hero> {
        ...
        return if (name.trim().isNotEmpty()) {
            heroes.forEach { hero ->
                if (hero.name.contains(name, ignoreCase = true)) {
                    founded.add(hero)
                }
            }
            founded
